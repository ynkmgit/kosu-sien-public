...
def test_create_project(clean_db):
    """プロジェクト作成"""
    project = ProjectService.create("TEST", "Test Project")
...
def test_update_project(clean_db):
    """プロジェクト更新"""
    created = ProjectService.create("TEST", "Test Project")
    updated = ProjectService.update(created["id"], "TEST2", "Updated Name")
...
def test_get_all_with_search(clean_db):
    """検索フィルタ"""
    ProjectService.create("SRCH", "UniqueSearchable")
    results = ProjectService.get_all(q="UniqueSearchable")
